...
import {useSupabase} from "@/context/SupabaseClientContext";

export function TextEditorToolbar({editor}: Readonly<{
    editor: Editor,
}>) {
    ...
    const browseImage = useCallback(() => {
        ...
        input.click();
    }, [editor, supabase.storage, currentImages]);

    const addImageByLink = () => {
        const url = (document.querySelector('#add-img-link-input') as HTMLInputElement).value;
        if (url) {
            ...
            >
                ...
                <DialogContent>
                    <DialogContent id="alert-dialog-description">
                        ...
                        <button onClick={browseImage} className={'mx-1 underline text-blue-500'}>
                            browse
                        </button>
                        or paste a link.

                        <div className={'flex mt-4 items-center space-x-2.5'}>
                            <div>Url:</div>
                            <input id={'add-img-link-input'}
                                   className={'w-full border-[1px] border-black rounded-lg p-1'}/>
                        </div>
                    </DialogContent>
                </DialogContent>
                <DialogActions>
                    <button onClick={addImageByLink} className={'p-2 rounded-lg hover:bg-gray-100 transition-all'}>Add
                    </button>
                </DialogActions>
            </Dialog>
